...
from causal_utility.utility_metrics import add_causal_errors, summarize_causal_utility, total_variation
...
def test_causal_error_and_summary():
    rows = pd.DataFrame({"scenario": ["standard"] * 2, "generator": ["g"] * 2, "estimator": ["aipw"] * 2, "estimate": [1.8, 2.2], "true_ate": [2.0, 2.0], "se": [0.2, 0.2], "ci_low": [1.4, 1.8], "ci_high": [2.2, 2.6]})
